...
use constant LIBDIR         => '/export/cgi/scripts/tfmail/';
use constant MAILPROG       => '/usr/lib/sendmail -oi -t';
use constant POSTMASTER     => 'formmail@gmoserver.jp';
...
use constant CHARSET        => 'Shift_JIS';
...
sub main
{
   ...
   if ( $ENV{REQUEST_METHOD} eq 'POST' )
   {
      ...
      check_referer($treq) or die "Bad referer";
...
This checks to make sure that the referer is set (i.e. there was a real web
page), and that it's coming from the right place.

=cut

sub check_referer
{
    return 0 unless $ENV{HTTP_REFERER};

    return 0 unless $ENV{HTTP_REFERER} =~ m!https?://(?:www\.)?([a-zA-Z0-9][a-zA-Z0-9.-]+)\.([a-zA-Z]+)/.*!;

    my ( $domain, $tld ) = ( $1, $2 );

    my $scriptname = $ENV{SCRIPT_FILENAME};

    return 0 unless $scriptname =~ m!/$tld/!;
    return 0 unless $scriptname =~ m!/(www.)?$domain/!;
    return 1;
}
...
sub check_recipients
{
   ...
   elsif ($treq->config('recipient') )
   {
      @recip = split /[\s,]+/, $treq->config('recipient', '');
   }
   elsif ( defined($treq->param('id')) && $treq->param('id') ne "" )
   {
       if($treq->param('id') !~ /\@/) {
           $recip[0] = $treq->param('id');
       }
   } elsif ( $ENV{"QUERY_STRING"} =~ /id=([^\?\/\&]*)/ ) {
       my $addr = $1;
       if($addr !~ /\@/) {
           $recip[0] = $1;
       } else {
           die 'invalid character (@) in id string';
       }
   }
...
EOERR
   ...
   print <<EOERR;
        <h1><div class="missing">送信エラー</div>
            送信に失敗しました。以下の原因が考えられます。</h1>

        <ul>
            <li>HTMLの記述に誤りがある</li>
            <li>契約外のサーバーから送信しようとした</li>
            <li>サーバー側でメンテナンス等が行われている</li>
        </ul>

        <p>解消しない場合は、お客様センターまでお問い合わせください。</p>
        $message
    </body>
</html>
EOERR
